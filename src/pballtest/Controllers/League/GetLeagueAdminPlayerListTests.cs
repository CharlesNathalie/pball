...
public partial class LeagueControllerTests : BaseControllerTests
{
    ...
    [InlineData("en-CA")]
    [InlineData("fr-CA")]
    public async Task GetLeagueAdminPlayerListAsync_Good_Test(string culture)
    {
                            ...
                            if (Configuration != null)
                            {
                                using (HttpClient httpClient = new HttpClient())
                                {
                                    ...
                                    if (contactLogin != null)
                                    {
                                        ...
                                        response = httpClient.GetAsync($"{ Configuration["pballurl"] }api/{ culture }/league/getleagueadminplayerlist/{league.LeagueID}").Result;
                                        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
    ...
    [InlineData("en-CA")]
    [InlineData("fr-CA")]
    public async Task GetLeagueAdminPlayerListAsync_Error_Test(string culture)
    {
        Assert.True(await LeagueControllerSetup(culture));
                    ...
                    if (db.Contacts != null)
                    {
                        ...
                        if (contact != null)
                        {
                            if (Configuration != null)
                            {
                                using (HttpClient httpClient = new HttpClient())
                                {
                                    HttpResponseMessage response = httpClient.GetAsync($"{ Configuration["pballurl"] }api/{ culture }/league/getleagueadminplayerlist/{league.LeagueID}").Result;
