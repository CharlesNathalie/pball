...
public partial class LeagueControllerTests : BaseControllerTests
{
                            ...
                            if (Configuration != null)
                            {
                                using (HttpClient httpClient = new HttpClient())
                                {
                                    ...
                                    if (contactLogin != null)
                                    {
                                        ...
                                        response = httpClient.GetAsync($"{ Configuration["pballurl"] }api/{ culture }/league/getuserleagues").Result;
                                        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
    ...
    [InlineData("en-CA")]
    [InlineData("fr-CA")]
    public async Task GetAllLeagueAsync_Error_Test(string culture)
    {
        Assert.True(await LeagueControllerSetup(culture));
                    ...
                    if (db.Contacts != null)
                    {
                        ...
                        if (contact != null)
                        {
                            if (Configuration != null)
                            {
                                using (HttpClient httpClient = new HttpClient())
                                {
                                    HttpResponseMessage response = httpClient.GetAsync($"{ Configuration["pballurl"] }api/{ culture }/league/getuserleagues").Result;
