...
export class ProfileService {
  ...
  GetErrorMessage(fieldName: 'ContactID' | 'LoginEmail' | 'FirstName' | 'Initial' | 'LastName' | 'PlayerLevel', form: FormGroup): string {
    switch (fieldName) {
      case 'ContactID':
        {
  ...
  GetHasError(fieldName: 'ContactID' | 'LoginEmail' | 'FirstName' | 'Initial' | 'LastName' | 'PlayerLevel', form: FormGroup): boolean {
  ...
  SubmitProfileForm(form: FormGroup) {
    if (form.valid) {
      this.Player.ContactID = form.controls['ContactID'].value;
